...
export async function createSocialLinksForCoaches() {
  ...
  for (const coach of final) {
    ...
    console.log(`   🔗 Created SocialLinks for ${coach.slug}`);
  }

  await fs.writeFile(COACH_DATA_DIR.SOCIAL_LINKS_MAP, JSON.stringify(socialMap, null, 2));
