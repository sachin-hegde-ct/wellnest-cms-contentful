...
export async function importTestimonials({ dryRun }: { dryRun: boolean }) {
  ...
  const finalTestimonials: unknown[] = [];
  ...
  for (let index = 0; index < sourceTestimonials.length; index++) {
     ...
     * 3. Persist state
     * ------------------------------------------------- */
    await writeDataFile(TESTIMONIAL_DATA_DIR.FINAL, finalTestimonials, {
      dryRun,
    });
    await writeDataFile(TESTIMONIAL_DATA_DIR.PROGRAM_MAP, programMap, {
      dryRun,
    });
